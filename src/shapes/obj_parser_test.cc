...
TEST(ObjParserTest, NegativeIndices) {
  ...
  EXPECT_EQ(parser.Vertex(-3), parser.Vertex(1));
  EXPECT_EQ(parser.Vertex(-2), parser.Vertex(2));
  EXPECT_EQ(parser.Vertex(-1), parser.Vertex(3));
  EXPECT_EQ(t1->p1(), parser.Vertex(1));
  EXPECT_EQ(t1->p2(), parser.Vertex(2));
  EXPECT_EQ(t1->p3(), parser.Vertex(3));

  EXPECT_EQ(parser.VertexNormal(-1), parser.VertexNormal(3));
  EXPECT_EQ(parser.VertexNormal(-3), parser.VertexNormal(1));
  EXPECT_EQ(parser.VertexNormal(-2), parser.VertexNormal(2));
  EXPECT_EQ(t1->n1(), parser.VertexNormal(3));
  EXPECT_EQ(t1->n2(), parser.VertexNormal(1));
  EXPECT_EQ(t1->n3(), parser.VertexNormal(2));
}

TEST(ObjParserTest, NegativeFacesParser) {
  auto s = "v -1 1 0\nv -1 0 0\nv 1 0 0\nv 1 1 0\nv 0 2 0\n\nf -5 -4 -3 -2 -1\n";
  auto ss = std::stringstream(s);
  ObjParser parser = ObjParser(ss);
...
TEST(ObjParserTest, NegativeVectorFacesParser) {
  auto s = "v 0 1 0\nv -1 0 0\nv 1 0 0\nvn -1 0 0\nvn 1 0 0\nvn 0 1 0\nf 1//-1 2//-3 3//-2\nf 1/0/-1 2/102/-3 3/14/-2\n";
  auto ss = std::stringstream(s);
  ObjParser parser = ObjParser(ss);
